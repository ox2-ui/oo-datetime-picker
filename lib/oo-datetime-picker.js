      ...
      if (self.type !== 'date') {
        ...
        self.ooAMPM = new Blaze.ReactiveVar(
          self.data.value ? moment.tz(self.data.value, self.data.timeZone).format('A') : (
            self.data.hour ? (self.data.hour > 12 ? 'PM' : 'AM') : 'AM'
          ));
...
// need as it fails otherwise
  ...
  self.autorun(function() {
      ...
      // if(self.ooYear.get() && self.ooMonth.get() && self.ooDay.get() && self.ooHours.get() && self.ooMinutes.get() && self.ooAMPM.get()) {
...
Template.ooDatetimePicker.helpers({
  ...
  activeMinute : function () {
   return typeof Template.instance().ooMinutes.get() === 'number' ? Template.instance().ooMinutes.get() : 'Not set'
  },
  //
  //      hour helpers
  //
  ...
  activeHour : function () {
    return typeof Template.instance().ooHours.get() === 'number' ? Template.instance().ooHours.get() : 'Not set'
  },
  ifActiveAMPM: function (params) {
    return params == Template.instance().ooAMPM.get() ? "is-active" : "";
  },
  //
  //      day helpers
  //
  ...
  years: function() {
    ...
    return  [yearSelected, yearSelected+1, yearSelected+ 2, yearSelected+3, yearSelected+4, yearSelected+5, yearSelected+6, yearSelected+7, yearSelected+8]
  },
...
Template.ooDatetimePicker.events({
  'click .js-closeAndSave': function(e, t) {
    var self = this;
    var query = {};
    query[self.field] = TimeWithISO(t.newValue.get());

    Collection[self.collection].update({_id: self.docId}, {$set: query}, function(err, res) {
      if (err)
        console.log(err)
    })
    hideDateSelectModal(function (res) {
      if (res) {
