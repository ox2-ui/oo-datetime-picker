...
getTimeFormatedInISO = function (curTime, timeZone) {
  // console.log('💣 curTime', curTime);
  var timeString = curTime.format("YYYY MMM D h:m A");
  // console.log('💣 timeString', timeString);
  return moment.tz(timeString, "YYYY MMM D h:m A", timeZone ? timeZone : Session.get('timeZone')).format();
  // return moment(preparedMoment).format();
...
Template.ooDatetimePicker.created = function () {
  ...
  if (self.data) {
      ...
      if (self.data.duration) {
        if (self.data.durationValue) {
          self.durationMinutes = new Blaze.ReactiveVar(self.data.durationValue.minutes);
          self.durationHours = new Blaze.ReactiveVar(self.data.durationValue.hours);
        } else {
      ...
      self.autoPM = new Blaze.ReactiveVar(self.data.autoPM)
      self.allowPastDates = new Blaze.ReactiveVar(self.data.allowPastDates)
    ...
    console.log('🍇', self.activeWidget.get());
...
// need as it fails otherwise
  self.autorun(function() {
    if (self.data.duration) {
      ...
      } else if (!self.ooMinutes.get() && self.ooMinutes.get() !== 0 ) {
      ...
      if (self.data.type !== 'time') {
        if (!self.ooYear.get()) {
          ...
          self.activeWidget.set('hourWidget');
        } else if (!self.ooMinutes.get() && self.ooMinutes.get() !== 0 && self.data.type !== 'date') {
          self.activeWidget.set('minuteWidget');
        }
      } else {
        self.activeWidget.set('hourWidget');
...
Template.ooDatetimePicker.helpers({
  ...
  pastDate: function() {
    if (!Template.instance().allowPastDates.get()) {
      const day = this;
      const month = Template.instance().ooMonth.get();
      const year = Template.instance().ooYear.get();
      const date = day && month && year ? moment(year + ' ' + month + ' ' + day, 'YYYY MMM D') : '';
      return moment().isAfter(date.add(1, 'days'))
    }
    return ''
  },
  ...
  durationHours : function () {
   return Template.instance().duration ? Template.instance().durationHours.get() : false;
  ...
  durationMinutes : function () {
   return Template.instance().duration ? Template.instance().durationMinutes.get() : false;
...
Template.ooDatetimePicker.events({
  ...
  'click .js-closeAndSave': function(e, t) {
    var self = this;
    var query = {};
    console.log('💣 t.newValue.get()', t.newValue.get());
    query[self.field] = getTimeFormatedInISO(t.newValue.get(), self.timeZone ? self.timeZone : false);
    if (self.duration) {
      query[self.fieldDuration] = {
            hours: t.durationHours.get(),
            minutes: t.durationMinutes.get()};
      if (self.endTime) {
        var addTime = moment(t.newValue.get()).add(query[self.fieldDuration]);
        console.log('💣 addTime', addTime);
        console.log('💣 self.fieldDuration', self.fieldDuration);
        query[self.fieldSecondary] = getTimeFormatedInISO(addTime, self.timeZone ? self.timeZone : false);
    ...
    if (typeof dateSelectModalHide !== 'undifined') {
      dateSelectModalHide(function (res) {
        if (res) {
          Session.set('DateSelectModal', false);
        }
  ...
  'click .js-selectHour' : function (e, t) {
    ...
    if (t.autoPM.get()) {
      ...
      if (pmHours.findIndex(x => x == this.valueOf()) >= 0) {
        t.ooAMPM.set('PM')
      }
    }
    t.activeWidget.set('minuteWidget');
  ...
  'click .js-durationMinutePlus' : function (e, t) {
    ...
    if (inc === 60) {
      if (t.durationHours.get() === 0 ){
        inc = 5
      } else {
        inc = 0
      }
    }
    t.durationMinutes.set(inc)
  },
  'click .js-durationMinuteMinus' : function (e, t) {
    ...
    if (inc === 0 && t.durationHours.get() === 0) {
      inc = 55
    }
    t.durationMinutes.set(inc)
  },
